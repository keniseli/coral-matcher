...
def pca_sklearn_gpu(input_tensor,feature_size, num_feats, num_components=3):
    # Ensure the input tensor is on the GPU
    device = input_tensor.device

    # Convert the input tensor to a NumPy array (bring it to CPU)
    input_numpy = input_tensor.cpu().numpy()

    # Reshape input to (H*W, num_feats)
    flattened_input = input_numpy.reshape(-1, num_feats)

    # Perform PCA using sklearn
    pca = PCA(n_components=num_components)
    reduced_input = pca.fit_transform(flattened_input)  # Shape will be (H*W, 3)

    output_numpy = reduced_input.reshape(feature_size[0], feature_size[0], num_components)

    # Convert back to PyTorch tensor and move back to the original device
    output_tensor = torch.tensor(output_numpy, device=device)
...
def save_labels(image_name, image_path, save_path, sparse_labels, tag=None, image_size=512):
...
def save_output_mask(similarity_mask, sparse_labels, image_name, save_path, image_size=512):
