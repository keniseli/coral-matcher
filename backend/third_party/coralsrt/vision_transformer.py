...
class VisionTransformer(nn.Module):
    ...
    def prepare_tokens(self, x):
        ...
        x = torch.cat((cls_tokens, x), dim=1) # B * num_patch * embedding_dim
...
class ViTFeat(nn.Module):
    ...
    def forward(self, img) :
        ...
        with torch.no_grad() :
            ...
            # layers=self.model.get_intermediate_layers(img,1)
            # features = layers[0][0,1:,:].reshape(feat_w*feat_h,-1).unsqueeze(0)
            # feats = torch.permute(features,(0,2,1))
