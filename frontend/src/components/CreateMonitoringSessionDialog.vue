<template>
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" @click.self="cancel">
        <div class="w-full max-w-md rounded-xl border border-slate-700 bg-[#0b181e] shadow-2xl">
            <!-- Header -->
            <div class="flex items-center justify-between border-b border-slate-800 px-5 py-4">
                ...
                <button type="button" class="text-xl text-slate-500 hover:text-slate-200" aria-label="Close"
                    @click="cancel">
                    ✕
                </button>
            </div>
            ...
            <form class="space-y-4 p-5" @submit.prevent="create">
                ...
                <label class="block text-sm">
                    ...
                    <input v-model="name" type="text" placeholder="Morning survey"
                        class="mt-1 w-full rounded border border-slate-700 bg-[#091419] p-2 text-sm text-slate-200 placeholder:text-slate-600 focus:border-teal-400 focus:outline-none" />
                </label>
                ...
                <label class="block text-sm">
                    ...
                    <select v-model="diveSite"
                        class="mt-1 block w-full rounded border border-slate-700 bg-[#091419] p-2 text-sm text-slate-200 focus:border-teal-400 focus:outline-none">
                ...
                <label class="block text-sm">
                    ...
                    <input v-model="timestamp" type="datetime-local" class="mt-1 w-full rounded border border-slate-700 bg-[#091419] p-2 text-sm 
                        text-slate-200 focus:border-teal-400 focus:outline-none
                ...
                <p v-if="error" class="rounded border border-red-400/40 bg-red-500/10 px-3 py-2 text-sm text-red-300">
                    ...
                    <div class="flex justify-end gap-2 border-t border-slate-800 pt-4">
                        ...
                        <button type="submit"
                            class="rounded bg-teal-400 px-4 py-2 text-sm font-semibold text-[#062126] disabled:opacity-40"
